c........+.........+.........+.........+.........+.........+.........+..
c  File Ceigenpbl.f contains the following 11 subroutines:  
c  Cespace, Cg, Cbal, Corth, Comqr, Comqr2, Cbabk2, Csroot, Cdiv, 
c  Pythaga and Csort.
c
c  Last modified: May 23, 2012.
c***********************************************************************
      subroutine cg(nm,n,ar,ai,wr,wi,matz,zr,zi,fv1,fv2,fv3,ierr)
c     Calls the recommended sequence of subroutines from the 
c     eigensystem subroutine package (eispack) to find the eigenvalues 
c     and eigenvectors (if desired) of a complex general matrix.
c     Input:
c      nm  must be set to the row dimension of the two-dimensional
c       array parameters as declared in the calling program dimension 
c       statement.
...
c     Translation of the algol procedure cbalance, which is a complex 
c     version of balance, Num. Math. 13, 293-304(1969) by Parlett and 
c     Reinsch. Handbook for auto. comp., vol.ii-linear algebra, 315-326
c     (1971).
c     Balances a complex matrix and isolates eigenvalues whenever 
c     possible.
c     Input
...
c     with j during the permutation step, and that the elements of the 
c     diagonal matrix used are denoted by d(i,j).  then 
...
c     Translation of a complex analogue of the algol procedure orthes, 
c     num. math. 12, 349-368(1968) by martin and wilkinson. handbook for 
c     auto. comp., vol.ii-linear algebra, 339-358(1971).
c     Given a complex general matrix, reduces a submatrix situated in 
c     rows and columns low through igh to upper hessenberg form by
...
c     calls pythaga for  dsqrt(a*a + b*b) .
c***********************************************************************

      implicit none
      integer i,j,m,n,ii,jj,la,mp,nm,igh,kp1,low
      double precision ar(nm,n),ai(nm,n),ortr(igh),orti(igh)
      double precision f,g,h,fi,fr,scale,pythaga
...
c     Translation of a unitary analogue of the algol procedure  comlr, 
c     num. math. 12, 369-376(1968) by martin and wilkinson. handbook for 
c     auto. comp., vol.ii-linear algebra, 396-403(1971). The unitary 
c     analogue substitutes the qr algorithm of francis (comp. jour. 4, 
c     332-345(1962)) for the lr algorithm.
c     Finds the eigenvalues of a complex upper hessenberg matrix by 
c     the qr method.
c     Input
c      nm must be set to the row dimension of two-dimensional
c       array parameters as declared in the calling program
...
c
      do 170 i = l, igh
         ...
         norm = pythaga(hr(i,i-1),hi(i,i-1))
         yr = hr(i,i-1) / norm
         yi = hi(i,i-1) / norm
         hr(i,i-1) = norm
         hi(i,i-1) = 0.0d0
...
c
      do 500 i = lp1, en
         sr = hr(i,i-1)
         hr(i,i-1) = 0.0d0
         norm = pythaga(pythaga(hr(i-1,i-1),hi(i-1,i-1)),sr)
         xr = hr(i-1,i-1) / norm
         wr(i-1) = xr
         xi = hi(i-1,i-1) / norm
         wi(i-1) = xi
...
c     Translation of a unitary analogue of the algol procedure comlr2, 
c     num. math. 16, 181-204(1970) by peters and wilkinson. handbook for 
c     auto. comp., vol.ii-linear algebra, 372-395(1971). the unitary 
c     analogue substitutes the qr algorithm of francis (comp. jour. 4, 
c     332-345(1962)) for the lr algorithm.
c     Finds the eigenvalues and eigenvectors of a complex upper 
c     hessenberg matrix by the qr method.  the eigenvectors of a complex 
c     general matrix can also be found if  corth  has been used to 
...
c     Translation of the algol procedure cbabk2, which is a complex 
c     version of balbak, num. math. 13, 293-304(1969) by parlett and 
c     reinsch. handbook for auto. comp., vol.ii-linear algebra, 315-326
c     (1971).
...
c***********************************************************************
      ...
      double precision s,tr,ti,pythaga
      tr = xr
      ti = xi
      s = dsqrt(0.5d0*(pythaga(tr,ti) + dabs(tr)))
      if (tr .ge. 0.0d0) yr = s
      if (ti .lt. 0.0d0) s = -s
      if (tr .le. 0.0d0) yi = s
...
c***********************************************************************
      double precision function pythaga(a,b)
...
c***********************************************************************
      subroutine csort(a, p, seval, sevec)
c     Sortespace takes three real eigenvalues a(3), with corresponding 
c     eigenvectors the columns of p(3,3). It rearranges the vector a 
c     into a descending order, so that a(1) > a(2) > a(3). The 
c     eigenvectors are rearranged at the same time to retain 
c     correspondence. Written by mark chapman 2002.
c***********************************************************************

      implicit none
      complex a(3), t1, t2, r1, r2, r3
      complex p(3,3), l1, l2, l3, seval(3), sevec(3,3)
      integer i, m, n

 10   if (real(a(1)).ge.real(a(2))) then
         t1=a(1)
         t2=a(2)
         a(1)=t2
         a(2)=t1
      ...
      if (real(a(1)).le.real(a(2)) .and. 
     +     real(a(2)).le.real(a(3))) then
         continue
      else
         goto 10
      end if 
