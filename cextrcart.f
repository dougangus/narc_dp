...
c***********************************************************************
      ...
      integer nt,it,nom,nom2,iom,kom,ic,jc,kc
      integer ik,iref,iout,im
      character a2text*4,a1text*17

      real*8 dt,tott,om,omk,omn,omwin,delom,tshift,pVp,pVs1,pVs2,pVs
      real*8 pi,dx,dx1,dx2,dx3,tx1,x1step,root2,vel
      real*8 phas,phas1,phas2,pphas1,pphas2,pphas,dpphas,slp,sls1,sls2
      real*8 a3r(3,3,3,3)
      real*8 slonor(3,ntmx)
     ...
     +     diag(3,3)
      ...
      complex*8 a3c(3,3,3,3)
      complex*8 fp0(3,3),fp2(3,3),fp3(3,3),fp22(3,3),fp23(3,3),
     +     fp32(3,3),fp33(3,3)
...
c     Evaluate frequency dependent homogeneous propagators
      do iom=1,nom2
         om=dble(iom-1)*delom
         call caijkl(aw6(1,1,iom),a3r,a3c)
         call cpropagat(a3c,fp0,fp2,fp3,fp22,fp23,fp32,fp33)
         pVp=real(fp0(1,1))
         pVs1=real(fp0(2,2))
         pVs2=real(fp0(3,3))
         pVs=max(pVs1,pVs2)
         do ic=1,3
            do jc=1,3
             cp0(ic,jc,iom)=dcmplx(fp0(ic,jc))
             cp2(ic,jc,iom)=dcmplx(fp2(ic,jc))
             cp3(ic,jc,iom)=dcmplx(fp3(ic,jc))
             cp22(ic,jc,iom)=dcmplx(fp22(ic,jc))
             cp23(ic,jc,iom)=dcmplx(fp23(ic,jc))
             cp32(ic,jc,iom)=dcmplx(fp32(ic,jc))
             cp33(ic,jc,iom)=dcmplx(fp33(ic,jc))
            enddo
         enddo
         do ic=1,3
            slonor(ic,iom)=cp0(ic,ic,iom)
         enddo
         if(iref.eq.1)then
            do ic=1,3
               do jc=1,3
                  if(ic.eq.jc)then
                     cp0(ic,jc,iom)=dcmplx(0.d0,0.d0)
                  endif
               enddo
            enddo
         endif
      ...
      do 10 ix1=1,nx1
         ...
         if(iwave.eq.1)then
            tshift=tshift+dx1*pVp
         elseif(iwave.ne.1)then
            tshift=tshift+dx1*pVs
         endif
     ...
     +           access='append')
            ...
            do jx2=1,nx2
               do jx3=1,nx3
                  ...
                  do it=1,nt
                     write(22)
     +                    uvec(1,it,jx3,jx2,kx1c)*cdexp(-ai*om*tshift),
     +                    uvec(2,it,jx3,jx2,kx1c)*cdexp(-ai*om*tshift),
     +                    uvec(3,it,jx3,jx2,kx1c)*cdexp(-ai*om*tshift)
                  enddo
               enddo
            enddo
            close(22)
